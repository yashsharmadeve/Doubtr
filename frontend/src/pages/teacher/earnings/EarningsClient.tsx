...
const payouts = [
  ...
  { id: "p3", date: "Mar 1, 2026", amount: "₹31,280", status: "Paid", method: "Bank · HDFC ••3421" },
];
  ...
  return (
    <div className="px-6 lg:px-10 py-8 max-w-7xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <Button variant="outline" className="rounded-xl"><Download className="h-4 w-4 mr-2" /> Export statement</Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-6 rounded-2xl border-border/70 bg-gradient-primary text-primary-foreground">
          ...
          <Button variant="secondary" className="mt-4 rounded-lg">Withdraw</Button>
        </Card>
        <Card className="p-6 rounded-2xl border-border/70">
          ...
          <div className="text-xs text-muted-foreground mt-1 flex items-center gap-1"><TrendingUp className="h-3 w-3" /> +18% vs last month</div>
        </Card>
        <Card className="p-6 rounded-2xl border-border/70">
          <div className="text-xs text-muted-foreground uppercase tracking-wider">Lifetime earnings</div>
        ...
        <div className="flex items-end gap-3 h-48">
          {months.map((m) => (
            <div key={m.m} className="flex-1 flex flex-col items-center gap-2">
              <div className="w-full bg-gradient-primary rounded-t-lg shadow-soft transition-all hover:opacity-90" style={{ height: `${(m.v / max) * 100}%` }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 p-6 rounded-2xl border-border/70">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="sm">See all <ArrowUpRight className="h-3.5 w-3.5 ml-1" /></Button>
          </div>
          <div className="divide-y divide-border/70">
            {payouts.map((p) => (
              <div key={p.id} className="py-4 flex items-center gap-4">
                <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center"><CreditCard className="h-4 w-4" /></div>
        ...
        <Card className="p-6 rounded-2xl border-border/70">
          ...
          <div className="mt-4 text-sm text-muted-foreground">Per question: <span className="font-medium text-foreground">₹40</span></div>
          <div className="text-sm text-muted-foreground">Per minute: <span className="font-medium text-foreground">₹6</span></div>
          <Button variant="outline" className="w-full mt-4 rounded-lg">Update payout details</Button>
        </Card>
      </div>
    </div>
  );
