...
import { ArrowLeft, ArrowRight, BookOpen, GraduationCap, Languages, Lock, Mail, Phone, Sparkles, User } from "lucide-react";
      ...
      <section>
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          <Field label="Full name" icon={User} required>
            <Input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Aarav Sharma" />
          </Field>
          ...
          <Field label="Email" icon={Mail} required>
            <Input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="you@school.com" />
          </Field>
      ...
      <section>
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          ...
          <Field label="Preferred language" icon={Languages}>
            ...
            >
              {LANGUAGES.map((l) => (
                <label key={l} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-border cursor-pointer hover:border-foreground/40 transition">
        ...
        <div className="mt-6 space-y-5">
          ...
          <Field label="Doubt type preference">
            <ChipGroup options={DOUBT_TYPES} values={form.doubtTypes} onToggle={(v) => toggle("doubtTypes", v)} />
          </Field>
        </div>
      </section>
